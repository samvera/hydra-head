...
RSpec::Matchers.define :have_solr_fields do |expected|
  match do |actual|
    ...
    elsif actual.is_a?(Solr::Document)
      inspected = actual.inspect
      result = false
      expected.each_pair do |field_name, field_value|
        /<Solr::Field.*@name=\"#{field_name.to_s}\".*, @value=\"#{field_value}\".*>/
        result = inspected.include?("@name=\"#{field_name}\", @boost=nil, @value=\"#{field_value}\"") || \
                 inspected.include?("@name=\"#{field_name}\", @value=\"#{field_value}\", @boost=nil") || \
                 inspected.include?("@value=\"#{field_value}\", @boost=nil, @name=\"#{field_name}\"") || \
                 inspected.include?("@value=\"#{field_value}\", @name=\"#{field_name}\", @boost=nil") || \
                 inspected.include?("@boost=nil, @value=\"#{field_value}\", @name=\"#{field_name}\"") || \
                 inspected.include?("@boost=nil, @name=\"#{field_name}\", @value=\"#{field_value}\"")
      end
     end
    result
  end
