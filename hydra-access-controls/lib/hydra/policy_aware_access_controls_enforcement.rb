...
module Hydra::PolicyAwareAccessControlsEnforcement
  ...
  def policy_clauses
    ...
    '(' + policy_ids.map { |id| ActiveFedora::SolrQueryBuilder.construct_query_for_rel(isGovernedBy: id) }.join(' OR ') + ')'
  end
  ...
  def apply_policy_group_permissions(permission_types = discovery_permissions)
    user_access_filters = []
    current_ability.user_groups.each_with_index do |group, _i|
