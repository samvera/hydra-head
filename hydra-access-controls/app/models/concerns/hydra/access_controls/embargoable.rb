  ...
  module AccessControls
    module Embargoable
      ...
      def embargo_visibility!
        ...
        if under_embargo?
          self.visibility_during_embargo = visibility_during_embargo || Hydra::AccessControls::AccessRight::VISIBILITY_TEXT_VALUE_PRIVATE
          self.visibility_after_embargo = visibility_after_embargo || Hydra::AccessControls::AccessRight::VISIBILITY_TEXT_VALUE_AUTHENTICATED
          self.visibility = visibility_during_embargo
        else
          self.visibility = visibility_after_embargo || Hydra::AccessControls::AccessRight::VISIBILITY_TEXT_VALUE_AUTHENTICATED
        end
      end
      ...
      def lease_visibility!
        if lease_expiration_date
          if active_lease?
            self.visibility_during_lease = visibility_during_lease || Hydra::AccessControls::AccessRight::VISIBILITY_TEXT_VALUE_AUTHENTICATED
            self.visibility_after_lease = visibility_after_lease || Hydra::AccessControls::AccessRight::VISIBILITY_TEXT_VALUE_PRIVATE
      ...
      def deactivate_embargo?(value)
        embargo&.embargo_release_date && value != embargo.visibility_during_embargo
      end

      # @return [true, false] true if there is a lease set up and the visibility will change
      def deactivate_lease?(value)
        lease&.lease_expiration_date && value != lease.visibility_during_lease
      end
    end
  end
end
