...
RSpec.describe RoleMapper do
  ...
  describe 'fetch_groups' do
    ...
    context 'for a user with multiple roles' do
      ...
      it { is_expected.to match_array %w[archivist donor patron] }

      it "doesn't change its response when it's called repeatedly" do
        expect(subject).to match_array %w[archivist donor patron]
        expect(RoleMapper.fetch_groups(user: user)).to match_array %w[archivist donor patron]
      end
    end
  ...
  describe 'roles' do
    ...
    it 'is quer[iy]able for roles for a given user' do
      expect(RoleMapper.roles('leland_himself@example.com').sort).to eq %w[archivist donor patron]
    ...
    context 'when called with a user instance' do
      ...
      it "doesn't change its response when it's called repeatedly" do
        expect(RoleMapper.roles(user).sort).to eq %w[archivist donor patron]
        expect(RoleMapper.roles(user).sort).to eq %w[archivist donor patron]
      end
    end
